...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //vincoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Vincoin
    // macOS: ~/Library/Application Support/Vincoin
    // Unix-like: ~/.vincoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vincoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Vincoin";
#else
    // Unix-like
    return pathRet / ".vincoin";
#endif
#endif
}
