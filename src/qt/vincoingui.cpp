...
const std::string VincoinGUI::DEFAULT_UIPLATFORM =
...
VincoinGUI::VincoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &VincoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &VincoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &VincoinGUI::showModalOverlay);
...
void VincoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Vincoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and vincoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &VincoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &VincoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &VincoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Vincoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Vincoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Vincoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Vincoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a vincoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Vincoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &VincoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &VincoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &VincoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &VincoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &VincoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &VincoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &VincoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &VincoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &VincoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &VincoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &VincoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &VincoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &VincoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &VincoinGUI::showDebugWindow);
}

void VincoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void VincoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &VincoinGUI::setCurrentWalletBySelectorIndex);
...
void VincoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &VincoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &VincoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &VincoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &VincoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void VincoinGUI::enableHistoryAction(bool privacy)
{
...
void VincoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &VincoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &VincoinGUI::removeWallet);
...
WalletController* VincoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void VincoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &VincoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &VincoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &VincoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &VincoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &VincoinGUI::incomingTransaction);
    connect(this, &VincoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void VincoinGUI::removeWallet(WalletModel* walletModel)
{
...
void VincoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void VincoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void VincoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void VincoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void VincoinGUI::createTrayIcon()
{
...
void VincoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &VincoinGUI::toggleHidden);
...
void VincoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void VincoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void VincoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void VincoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void VincoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void VincoinGUI::openClicked()
{
...
void VincoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void VincoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void VincoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void VincoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void VincoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void VincoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void VincoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void VincoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Vincoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void VincoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void VincoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void VincoinGUI::updateHeadersSyncProgressLabel()
{
...
void VincoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void VincoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &VincoinGUI::quitRequested);
...
void VincoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void VincoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &VincoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void VincoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void VincoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &VincoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &VincoinGUI::show);
                e->ignore();
            }
        }
    }
...
void VincoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void VincoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void VincoinGUI::incomingTransaction(const QString& date, VincoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(VincoinUnits::formatWithUnit(unit, amount, true));
...
void VincoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void VincoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool VincoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool VincoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void VincoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void VincoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void VincoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void VincoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void VincoinGUI::updateWindowTitle()
{
...
void VincoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void VincoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VincoinGUI::detectShutdown()
{
...
void VincoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void VincoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(VincoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = VincoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void VincoinGUI::subscribeToCoreSignals()
{
...
void VincoinGUI::unsubscribeFromCoreSignals()
{
...
bool VincoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<VincoinUnit> units = VincoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const VincoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, VincoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const VincoinUnit u : VincoinUnits::availableUnits()) {
        menu->addAction(VincoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(VincoinUnit newUnits)
{
    setText(VincoinUnits::longName(newUnits));
}
