# CJDNS support in Vincoin Core

It is possible to run Vincoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Vincoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Vincoin network and individual nodes.
...
before it will work with your Vincoin Core node. This is described in steps
...
## Run Vincoin Core with CJDNS

Once you are connected to the CJDNS network, the following Vincoin Core
...
When enabled, this option tells Vincoin Core that it is running in an
...
IPv6 local network. This helps Vincoin Core perform better address management:
...
CJDNS support was added to Vincoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `vincoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/vincoin/vincoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Vincoin Core

There are several ways to see your CJDNS address in Vincoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `vincoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `vincoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `vincoin-cli help getnodeaddresses` for details.
