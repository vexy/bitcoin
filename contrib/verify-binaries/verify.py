...
"""Script for verifying Vincoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from vincoincore.org and vincoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/vincoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://vincoincore.org"
HOST2 = "https://vincoin.org"
VERSIONPREFIX = "vincoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Vincoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"vincoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by vincoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the vincoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes vincoin.org lags behind vincoincore.org.)')
    )
