...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing vincoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='vincoin-cli', help='vincoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Vincoin address to which the faucet should send')
...
parser.add_argument('vincoin_cli_args', nargs='*', help='Arguments to pass on to vincoin-cli (default: -signet)')

args = parser.parse_args()

if args.vincoin_cli_args == []:
    args.vincoin_cli_args = ['-signet']


def vincoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.vincoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = vincoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = vincoin_cli(['getnewaddress', 'faucet', 'bech32'])
