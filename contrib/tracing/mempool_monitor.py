...
""" Example logging Vincoin Core mempool events using the mempool:added,
...
def main(vincoind_path):
    vincoind_with_usdts = USDT(path=str(vincoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    vincoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    vincoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    vincoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    vincoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[vincoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/vincoind")
        exit(1)
