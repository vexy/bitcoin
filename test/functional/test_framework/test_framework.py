...
TMPDIR_PREFIX = "vincoin_func_test_"
...
class VincoinTestMetaClass(type):
    """Metaclass for VincoinTestFramework.

    Ensures that any attempt to register a subclass of `VincoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'VincoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("VincoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("VincoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class VincoinTestFramework(metaclass=VincoinTestMetaClass):
    """Base class for a vincoin test script.

    Individual vincoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave vincoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop vincoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use vincoin-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: vincoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("VincoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'vincoind', self.options.vincoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'vincoin-cli', self.options.vincoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a vincoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple vincoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the vincoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as vincoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    vincoind=self.options.vincoind,
                    vincoin_cli=self.options.vincoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in vincoin.conf
    ...
    def skip_if_no_vincoind_tracepoints(self):
        """Skip the running test if vincoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("vincoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_vincoind_zmq(self):
        """Skip the running test if vincoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("vincoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if vincoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("vincoin-wallet has not been compiled")

    def skip_if_no_vincoin_util(self):
        """Skip the running test if vincoin-util has not been compiled."""
        if not self.is_vincoin_util_compiled():
            raise SkipTest("vincoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if vincoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("vincoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether vincoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether vincoin-wallet was compiled."""
